...
def _gather_config_files(package=None):
    ...
    # add system config /etc/config.ini
    sys_config = os.path.abspath(os.path.join(os.sep, "etc", "config.ini"))
    if os.path.isfile(sys_config):
        conf_files.append(sys_config)

    PICKLE_CONFIG = "PICKLE_CONFIG"
    if PICKLE_CONFIG in os.environ:
        conf_files.extend(os.environ[PICKLE_CONFIG].split(":"))
